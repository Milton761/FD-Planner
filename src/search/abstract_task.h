...
public:
    ...
    virtual float get_operator_cost(int index, bool is_axiom) const = 0;
    ...
    virtual std::pair<int, int> get_operator_effect(
        int op_index, int eff_index, bool is_axiom) const = 0;
    virtual const GlobalOperator *get_global_operator(int index,
            bool is_axiom) const = 0;
    ...
    virtual std::vector<int> get_state_values(const GlobalState &global_state) const
        = 0;
};
