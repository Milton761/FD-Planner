...
float SearchNode::get_g() const
{
    assert(info.g >= 0);
    return info.g;
}

float SearchNode::get_real_g() const
{
    return info.real_g;
}

float SearchNode::get_h() const
{
    return info.h;
}
...
void SearchNode::open_initial(float h)
{
...
void SearchNode::open(float h, const SearchNode &parent_node,
                      const GlobalOperator *parent_op, float cost)
{
    assert(info.status == SearchNodeInfo::NEW);
    info.status = SearchNodeInfo::OPEN;
    info.g = parent_node.info.g + cost;
...
void SearchNode::increase_h(float h)
{
    assert(h >= info.h);
    info.h = h;
}
...
{
    return SearchNode(state.get_id(), search_node_infos[state]);
}
