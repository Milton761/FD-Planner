...
public:
    ...
    virtual void generate_applicable_ops(
        const GlobalState &state, vector<OperatorProxy> &applicable_ops) const = 0;
    virtual void generate_applicable_ops(
        const State &state, vector<OperatorProxy> &applicable_ops) const = 0;
    // Transitional method, used until the search is switched to the new task interface.
    virtual void generate_applicable_ops(
        const GlobalState &state,
        vector<const GlobalOperator *> &applicable_ops) const = 0;
};
...
public:
    ...
    GeneratorSwitch(const VariableProxy &switch_var,
                    ...
                    GeneratorBase *default_generator);
    virtual void generate_applicable_ops(
        const GlobalState &state, vector<OperatorProxy> &applicable_ops) const;
    virtual void generate_applicable_ops(
...
void GeneratorLeaf::generate_applicable_ops(
    const GlobalState &, vector<OperatorProxy> &applicable_ops) const
{
    applicable_ops.insert(applicable_ops.end(),
...
void GeneratorEmpty::generate_applicable_ops(
    const GlobalState &, vector<OperatorProxy> &) const
{
}
...
void SuccessorGenerator::generate_applicable_ops(
    const GlobalState &state,
    std::vector<const GlobalOperator *> &applicable_ops) const
{
    root->generate_applicable_ops(state, applicable_ops);
}
