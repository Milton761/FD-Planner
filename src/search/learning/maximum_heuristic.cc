...
float MaxHeuristic::compute_heuristic(const GlobalState &state)
{
    int max = 0;
    bool dead_end = false;
    ++num_evals;
...
void MaxHeuristic::print_statistics() const
{
    ...
    for (size_t i = 0; i < heuristics.size(); ++i) {
        cout << i /*heuristics[i]->get_name()*/ << " , " << winner[i] << " , " <<
             only_winner[i] << endl;
    }
}

bool MaxHeuristic::reach_state(const GlobalState &parent_state,
