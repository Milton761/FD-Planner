    ...
    for (int i = 0; i < num_heuristics; ++i) {
        for (int j = i + 1; j < num_heuristics; ++j) {
            ...
            cout << expensive[classifier_index] /*heuristics[expensive[classifier_index]]->get_name()*/
                 ...
                 << " - " <<
                 cheap[classifier_index] /*heuristics[cheap[classifier_index]]->get_name()*/ <<
                 " > " << thr << endl;
...
int SelectiveMaxHeuristic::eval_heuristic(const GlobalState &state, int index,
        bool count)
{
    if (count) {
        ++num_evaluated[index];
...
float SelectiveMaxHeuristic::compute_heuristic(const GlobalState &state)
{
    ++num_evals;
    dead_end = false;
    ...
    */
    ...
    if (test_mode) {
        ...
        for (int classifier_index = 0; classifier_index < num_pairs;
             ++classifier_index) {
            int diff = hvalue[expensive[classifier_index]] -
                       hvalue[cheap[classifier_index]];
...
bool SelectiveMaxHeuristic::reach_state(const GlobalState &parent_state,
...
void SelectiveMaxHeuristic::print_statistics() const
{
    ...
    if (test_mode) {
        ...
        double acc =
            ...
            (double)(correct0_classifications + correct1_classifications +
                     incorrect0_classifications + incorrect1_classifications);
    ...
    double eval_time = 0;
    cout << "heuristic,  evaluated, winner,   only winner, total time, average time"
         << endl;
    for (size_t i = 0; i < heuristics.size(); ++i) {
