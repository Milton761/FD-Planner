...
void AdditiveHeuristic::relaxed_exploration()
{
    ...
    while (!queue.empty()) {
        pair<float, Proposition *> top_pair = queue.pop();
        float distance = top_pair.first;
        Proposition *prop = top_pair.second;
        float prop_cost = prop->cost;
        assert(prop_cost >= 0);
...
float AdditiveHeuristic::compute_add_and_ff(const State &state)
{
    ...
    for (size_t i = 0; i < goal_propositions.size(); ++i) {
        float prop_cost = goal_propositions[i]->cost;
        if (prop_cost == -1) {
            return DEAD_END;
        }
...
float AdditiveHeuristic::compute_heuristic(const GlobalState &global_state)
{
    State state = convert_global_state(global_state);
    float h = compute_add_and_ff(state);
    if (h != DEAD_END) {
        for (size_t i = 0; i < goal_propositions.size(); ++i) {
