        ...
        bool level_out, const LandmarkNode *exclude, bool compute_lvl_op) const
{
    ...
    if (compute_lvl_op) {
        ...
        for (size_t i = 0; i < g_operators.size() + g_axioms.size(); ++i) {
            ...
            lvl_op[i] = unordered_map<pair<int, int>, float> ();
    ...
    lm_graph->get_exploration()->compute_reachability_with_excludes(lvl_var, lvl_op,
            level_out,
...
bool LandmarkFactory::effect_always_happens(const vector<GlobalEffect> &effects,
        set <
        ...
        if (effect_conditions.find(effects[i].var) != effect_conditions.end()
            ...
            for (size_t j = 0; j < effects[i].conditions.size(); ++j) {
                vector<pair<int, int> > &vec = effect_conditions.find(
                                                   effects[i].var)->second.second;
                vec.push_back(make_pair(effects[i].conditions[j].var,
                                        ...
                                        unordered_set<LandmarkNode *> &acyclic_node_set)
{
    ...
    unordered_set<LandmarkNode *> visited = unordered_set<LandmarkNode *>
...
float LandmarkFactory::calculate_lms_cost() const
{
    float result = 0;
    for (set<LandmarkNode *>::const_iterator it = lm_graph->get_nodes().begin(); it
         != lm_graph->get_nodes().end(); ++it) {
...
void LandmarkFactory::compute_predecessor_information(
    LandmarkNode *bp,
    vector<vector<int> > &lvl_var,
    std::vector<std::unordered_map<std::pair<int, int>, float> > &lvl_op)
{
