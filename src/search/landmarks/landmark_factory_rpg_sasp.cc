...
void LandmarkFactoryRpgSasp::get_greedy_preconditions_for_lm(
    const LandmarkNode *lmp, const GlobalOperator &o,
...
float LandmarkFactoryRpgSasp::min_cost_for_landmark(LandmarkNode *bp,
        vector < vector <
        int > > &lvl_var)
{
    ...
    for (size_t i = 0; i < bp->vars.size(); ++i) {
        ...
        for (size_t j = 0; j < ops.size(); ++j) {
            ...
            if (_possibly_reaches_lm(op, lvl_var, bp)) {
                min_cost = min(min_cost, lm_graph->get_adjusted_action_cost(op));
            }
        }
    }
    /*
      ...
      See issue 467
    */
    assert(min_cost < numeric_limits<float>::max());
    return min_cost;
}
...
void LandmarkFactoryRpgSasp::found_disj_lm_and_order(const set<pair<int, int> >
        a,
        LandmarkNode &b, edge_type t)
{
        ...
        const unordered_set<int> &exclude)
{
    ...
    assert(landmark.second !=
           initial_state[landmark.first]); // no initial state landmarks
