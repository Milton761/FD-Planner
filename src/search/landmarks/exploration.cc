...
void Exploration::increase_cost(float &cost, float amount)
{
    assert(cost >= 0);
    assert(amount >= 0);
    cost += amount;
...
void Exploration::set_additional_goals(const vector<pair<int, int> >
                                       &add_goals)
{
...
void Exploration::build_unary_operators(const GlobalOperator &op)
{
    ...
    float base_cost = get_adjusted_cost(op);
    ...
    for (size_t i = 0; i < effects.size(); ++i) {
        ...
        unary_operators.push_back(ExUnaryOperator(precondition, effect, &op,
                                  base_cost));
        precondition.clear();
                                      ...
                                      bool level_out = false)
{
    ...
    while (!prop_queue.empty()) {
        pair<float, ExProposition *> top_pair = prop_queue.pop();
        float distance = top_pair.first;
...
void Exploration::enqueue_if_necessary(ExProposition *prop, float cost,
                                       float depth,
                                       ExUnaryOperator *op,
                                       bool use_h_max)
{
...
float Exploration::compute_hsp_add_heuristic()
{
    float total_cost = 0;
    for (size_t i = 0; i < goal_propositions.size(); ++i) {
        float prop_cost = goal_propositions[i]->h_add_cost;
        if (prop_cost == -1) {
            return DEAD_END;
        }
...
float Exploration::compute_ff_heuristic(const GlobalState &state)
{
    float h_add_heuristic = compute_hsp_add_heuristic();
...
void Exploration::compute_reachability_with_excludes(vector<vector<int> >
        &lvl_var,
        vector<unordered_map<pair<int, int>, float> > &lvl_op,
        bool level_out,
        const vector<pair<int, int> > &excluded_props,
        const unordered_set<const GlobalOperator *> &excluded_ops,
        bool compute_lvl_ops)
{
    ...
    if (compute_lvl_ops) {
        ...
        for (size_t i = 0; i < unary_operators.size(); ++i) {
            ...
            if (op.h_max_cost == numeric_limits<float>::max()) {
                break;
            }
...
float Exploration::compute_heuristic(const GlobalState &state)
{
                                ...
                                const GlobalState &state)
{
    ...
    if (!landmarks.empty()) {
        ...
        for (size_t i = 0; i < termination_propositions.size(); ++i) {
            const float prop_cost = termination_propositions[i]->h_add_cost;
