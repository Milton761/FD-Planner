...
struct ExUnaryOperator {
    ...
    float base_cost; // 0 for axioms, 1 for regular operators
...
{
    constexpr static const float MAX_COST_VALUE =
        100000000; // See additive_heuristic.h.
    ...
    HeapQueue<float, ExProposition *> prop_queue;
    ...
    float compute_hsp_add_heuristic();
    float compute_ff_heuristic(const GlobalState &state);

    void collect_ha(ExProposition *goal, RelaxedPlan &relaxed_plan,
                    const GlobalState &state);

    void enqueue_if_necessary(ExProposition *prop, float cost, float depth,
                              ExUnaryOperator *op,
                              bool use_h_max);
