      ...
      lm_status_manager(lgraph)
{
    ...
    if (opts.get<bool>("admissible")) {
        ...
        if (lgraph.is_using_reasonable_orderings()) {
            cerr << "Reasonable orderings should not be used for admissible heuristics" <<
                 endl;
            exit_with(EXIT_INPUT_ERROR);
        } else if (has_axioms()) {
            cerr << "cost partitioning does not support axioms" << endl;
            exit_with(EXIT_UNSUPPORTED);
        } else if (has_conditional_effects()
                   && !lgraph.supports_conditional_effects()) {
            cerr << "conditional effects not supported by the landmark generation method" <<
                 endl;
            exit_with(EXIT_UNSUPPORTED);
        }
...
void LandmarkCountHeuristic::set_exploration_goals(const GlobalState &state)
{
    ...
    LandmarkSet result;
    const vector<bool> &reached_lms_v = lm_status_manager.get_reached_landmarks(
                                            state);
...
float LandmarkCountHeuristic::get_heuristic_value(const GlobalState &state)
{
    //double epsilon = 0.01;
    ...
    if (use_cost_sharing) {
        ...
        h = (float)lm_cost_assignment->cost_sharing_h_value();
    } else {
        lgraph.count_costs();

        float total_cost = lgraph.cost_of_landmarks();
        float reached_cost = lgraph.get_reached_cost();
        float needed_cost = lgraph.get_needed_cost();
...
float LandmarkCountHeuristic::compute_heuristic(const GlobalState &state)
{
    ...
    float h = get_heuristic_value(state);
