                                     ...
                                     const FluentSet &superset)
{
    ...
    for (size_t i = 0; i < current.size(); ++i) {
        if (!interesting(superset[current_var_index].first,
                         superset[current_var_index].second,
    ...
    if (use_var) {
        ...
        get_m_sets_of_set_(m, num_included + 1, current_var_index + 1, current, subsets,
                           superset);
        current.pop_back();
    }
...
bool HMLandmarks::possible_noop_set(const FluentSet &fs1, const FluentSet &fs2)
{
    ...
    for (fs1it = fs1.begin(); fs1it != fs1.end(); ++fs1it) {
        for (fs2it = fs2.begin(); fs2it != fs2.end(); ++fs2it) {
            if (are_mutex(make_pair(fs1it->first, fs1it->second), make_pair(fs2it->first,
                          fs2it->second))) {
                return false;
            }
        }
...
void HMLandmarks::generate_landmarks()
{
    ...
    for (size_t i = 0; i < goal_subsets.size(); ++i) {
        ...
        if (h_m_table_[set_index].level == -1) {
            std::cout << std::endl << std::endl << "Subset of goal not reachable !!." <<
