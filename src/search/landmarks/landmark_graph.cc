...
bool LandmarkGraph::exact_same_disj_landmark_exists(const set<pair<int, int> >
        &lm) const
{
...
LandmarkNode &LandmarkGraph::landmark_add_disjunctive(const set<pair<int, int> >
        &lm)
{
    vector<int> vars;
    vector<int> vals;
...
LandmarkNode &LandmarkGraph::landmark_add_conjunctive(const set<pair<int, int> >
        &lm)
{
    vector<int> vars;
    vector<int> vals;
...
void LandmarkGraph::add_options_to_parser(OptionParser &parser)
{
    ...
    parser.add_option<shared_ptr<AbstractTask> >("transform", "",
            OptionParser::NONE);
}
