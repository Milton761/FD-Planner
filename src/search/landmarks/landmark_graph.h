...
public:
    LandmarkNode(std::vector<int> &variables, std::vector<int> &values, bool disj,
                 bool conj = false)
        : id(-1), vars(variables), vals(values), disjunctive(disj), conjunctive(conj),
          in_goal(false),
...
public:
    ...
    inline float cost_of_landmarks() const {
        return landmarks_cost;
    }
    void count_costs();
    LandmarkNode *get_lm_for_index(int);
    float get_needed_cost() const {
        return needed_cost;
    }
    float get_reached_cost() const {
        return reached_cost;
    }
    ...
    bool is_using_reasonable_orderings() const {
    ...
    inline const std::vector<int> &get_operators_including_eff(
        const std::pair<int, int> &eff) const {
        return operators_eff_lookup[eff.first][eff.second];
    }

    bool use_orders() const {
        return !no_orders;    // only needed by HMLandmark
    }
    bool use_only_causal_landmarks() const {
        return only_causal_landmarks;
    }
    bool use_disjunctive_landmarks() const {
        return disjunctive_landmarks;
    }
    bool use_conjunctive_landmarks() const {
        return conjunctive_landmarks;
    }
    ...
    bool simple_landmark_exists(const std::pair<int, int> &lm)
    const; // not needed by HMLandmark
    bool disj_landmark_exists(const std::set<std::pair<int, int> > &lm)
    const; // not needed by HMLandmark
    bool landmark_exists(const std::pair<int, int> &lm)
    const; // not needed by HMLandmark
    bool exact_same_disj_landmark_exists(const std::set<std::pair<int, int> > &lm)
    const;

    LandmarkNode &landmark_add_simple(const std::pair<int, int> &lm);
    LandmarkNode &landmark_add_disjunctive(const std::set<std::pair<int, int> >
                                           &lm);
    LandmarkNode &landmark_add_conjunctive(const std::set<std::pair<int, int> >
                                           &lm);
    void rm_landmark_node(LandmarkNode *node);
    LandmarkNode &make_disj_node_simple(std::pair<int, int>
                                        lm); // only needed by LandmarkFactorySasp
    void set_landmark_ids();
    void set_landmark_cost(float cost) {
        landmarks_cost = cost;
    }
    void dump_node(const LandmarkNode *node_p) const;
    void dump() const;
    float get_adjusted_action_cost(const GlobalOperator &op) {
        return task->get_operator_cost(op.get_id(), op.is_axiom());
    }
private:
