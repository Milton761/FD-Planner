...
GlobalEffect::GlobalEffect(int variable, int value,
                           const vector<GlobalCondition> &conds)
    : var(variable),
      val(value),
      conditions(conds)
{
...
GlobalOperator::GlobalOperator(int id, float cost) : id(id), cost(cost)
{
    marked = false;
    is_an_axiom = false;
    name = "dummy";
...
GlobalOperator::GlobalOperator(int id, istream &in, bool axiom) : id(id)
{
    marked = false;
