...
class AbstractQueueGeneral
{
public:
    typedef std::pair<Key, Value> Entry;

    AbstractQueueGeneral() {}
    virtual ~AbstractQueueGeneral() {}
    ...
    virtual AbstractQueueGeneral<Key, Value> *convert_if_necessary(Key /*key*/) {
...
template<typename Key, typename Value>
class HeapQueue: public AbstractQueueGeneral<Key, Value>
{
    typedef typename AbstractQueueGeneral<Key, Value>::Entry Entry;
...
class AbstractQueue : public AbstractQueueGeneral<int, Value> {};
...
public:
    ...
    virtual AbstractQueueGeneral<int, Value> *convert_if_necessary(int key) {
        if (key >= MIN_BUCKETS_BEFORE_SWITCH && key > num_pushes) {
            ...
            return HeapQueue<int, Value>::create_from_sorted_entries_destructively(
                       entries);
        }
        return this;
    }
...
class AdaptiveQueue
{
    AbstractQueueGeneral<int, Value> *wrapped_queue;
...
public:
    ...
    void push(int key, const Value &value) {
        AbstractQueueGeneral<int, Value> *q = wrapped_queue->convert_if_necessary(key);
        if (q != wrapped_queue) {
            delete wrapped_queue;
            wrapped_queue = q;
        }
