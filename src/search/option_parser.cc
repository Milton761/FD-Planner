...
static void parse_initial_budget(std::string s, bool /*dry_run*/)
{
    std::ifstream inf(s.c_str());
    if (inf.good()) {
        std::string l;
        if (std::getline(inf, l)) {
            g_budget = atoi(l.c_str());
        }
    } else {
        g_budget = atoi(s.c_str());
    }
}
    ...
    const vector<string> &args, bool dry_run)
{
    ...
    for (size_t i = 0; i < args.size(); ++i) {
        ...
        } else if (arg.compare("--budget") == 0) {
            if (is_last) {
                throw ArgError("missing argument after --budget");
            }
            ++i;
            parse_initial_budget(args[i], false);
        } else if (arg.compare("--criterion") == 0) {
            if (is_last) {
                throw ArgError("missing argument after --criterion");
            }
            ++i;
            if (args[i].compare("maxprob") == 0) {
                g_optimization_criterion = OptimizationCriterion::MAXP;
            } else if (args[i].compare("expcost") == 0) {
                g_optimization_criterion = OptimizationCriterion::EXPC;
            } else {
                throw ArgError("unknown optimization criterion " + args[i]);
            }
        } else if (arg.compare("--giveup") == 0) {
            if (is_last) {
                throw ArgError("missing argument after --criterion");
            }
            ++i;
            g_giveup = parse_int_arg("--giveup", args[i]);
        } else if (arg.compare("--store-policy") == 0) {
            g_store_policy = true;
        } else if (arg.compare("--dual") == 0) {
            g_use_upper_and_lower = true;
        } else {
            throw ArgError("unknown option " + arg);
        }
    }
    if (search_pos < args.size()) {
        OptionParser p(args[search_pos], dry_run);
...
string OptionParser::usage(string progname)
{
    string usage =
        ...
        "--budget BUDGET\n"
        "    This will set the budget limit to BUDGET. BUDGET = -1 is considered to be infinity\n\n"
        "--criterion X\n"
        "    This will set the optimization criterion to X.\n"
        "    Currently, only X = maxprob and X = expcost are implemented.";
    return usage;
}
...
static ParseTree generate_parse_tree(string config)
{
    ...
    for (size_t i = 0; i < config.size(); ++i) {
        ...
        case '[':
            if (!buffer.empty()) {
                throw ParseError("misplaced opening bracket [", *cur_node, config.substr(0,
                                 i));
            }
