...
public:
    EffectConditionsProxy(
        ...
        : ConditionsProxy(task), op_index(op_index), eff_index(eff_index),
          is_axiom(is_axiom) {}
...
public:
    ...
    virtual bool operator==(const OperatorProxy &other) const {
        assert(task == other.task);
    ...
    virtual PreconditionsProxy get_preconditions() const {
