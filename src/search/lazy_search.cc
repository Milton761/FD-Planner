...
SearchStatus LazySearch::fetch_next_state()
{
    ...
    GlobalState current_predecessor = g_state_registry->lookup_state(
                                          current_predecessor_id);
    assert(current_operator->is_applicable(current_predecessor));
    current_state = g_state_registry->get_successor_state(current_predecessor,
                    *current_operator);
    ...
    */
    current_eval_context = EvaluationContext(current_state, current_g, true,
                           statistics);
    ...
    if (node.is_new() || reopen) {
        ...
        if (!open_list->is_dead_end(current_eval_context)) {
            ...
            if (reopen) {
                ...
                node.open(h, parent_node, current_operator, get_adjusted_cost(*current_operator));
            }
            node.close();
